...
ORBextractor::ORBextractor(int features_num, float scale_factor, int levels_num,
		...
		features_num_(features_num), scale_factor_(scale_factor), levels_num_(
				levels_num), default_fast_threshold_(default_fast_threshold), min_fast_threshold_(
				min_fast_threshold) {
	// 这边将每层金字塔对应的尺度因子给出
	...
	feature_num_per_level_[levels_num_ - 1] = std::max(
			features_num_ - sum_features, 0);
...
static void computeOrientation(const cv::Mat& image,
		std::vector<cv::KeyPoint>& keypoints, const std::vector<int>& umax) {
	for (std::vector<cv::KeyPoint>::iterator keypoint = keypoints.begin(),
			keypoint_end = keypoints.end(); keypoint != keypoint_end;
			++keypoint) {
...
std::vector<cv::KeyPoint> ORBextractor::distributeQuadTree(
		const std::vector<cv::KeyPoint>& vec_to_distribute_keys,
		const int &min_x, const int &max_x, const int &min_y, const int &max_y,
		const int &feature_num, const int &level) {
	float ceshi = (static_cast<float>(max_x - min_x) / (max_y - min_y));
	// 计算初始时有几个节点
	const int init_node_num = (int) (static_cast<float>(max_x - min_x)
			/ (max_y - min_y) + 0.5);
	// 得到节点之间的间隔
	...
	while (!is_finish) {
			...
			if (lit->is_no_more_) {
				...
				if (n1.vec_keys_.size() > 0) {
					...
					if (n1.vec_keys_.size() > 1) {
						...
						keys_size_and_node.push_back(
								std::make_pair(n1.vec_keys_.size(),
										&list_nodes.front()));
				...
				if (n2.vec_keys_.size() > 0) {
					...
					if (n2.vec_keys_.size() > 1) {
						to_expand_num++;
						keys_size_and_node.push_back(
								std::make_pair(n2.vec_keys_.size(),
										&list_nodes.front()));
						list_nodes.front().node_iter_ = list_nodes.begin();
					}
				}
				if (n3.vec_keys_.size() > 0) {
					...
					if (n3.vec_keys_.size() > 1) {
						to_expand_num++;
						keys_size_and_node.push_back(
								std::make_pair(n3.vec_keys_.size(),
										&list_nodes.front()));
						list_nodes.front().node_iter_ = list_nodes.begin();
					}
				}
				if (n4.vec_keys_.size() > 0) {
					...
					if (n4.vec_keys_.size() > 1) {
						to_expand_num++;
						keys_size_and_node.push_back(
								std::make_pair(n4.vec_keys_.size(),
										&list_nodes.front()));
						list_nodes.front().node_iter_ = list_nodes.begin();
					}
				}
		...
		if ((int) list_nodes.size() >= feature_num
			...
			while (!is_finish) {
				...
				std::vector<std::pair<int, ExtractorNode*> > prev_size_and_node =
						keys_size_and_node;
				keys_size_and_node.clear();
	...
	for (std::list<ExtractorNode>::iterator lit = list_nodes.begin();
			lit != list_nodes.end(); lit++) {
...
void ORBextractor::computeKeyPointsQuadTree(
	...
	for (int level = 0; level < levels_num_; ++level) {
		...
		for (int i = 0; i < rows; i++) {
			...
			for (int j = 0; j < cols; j++) {
				...
				cv::FAST(
						vec_image_pyramid_[level].rowRange(ini_y, max_y).colRange(
								ini_x, max_x), vec_keys_cell,
						default_fast_threshold_, true);
				...
				if (!vec_keys_cell.empty()) {
					for (std::vector<cv::KeyPoint>::iterator vit =
							vec_keys_cell.begin(); vit != vec_keys_cell.end();
							vit++) {
		...
		// 将特征点进行四叉树划分
		keypoints = distributeQuadTree(vec_to_distribute_keys, min_border_x,
				max_border_x, min_border_y, max_border_y,
				feature_num_per_level_[level], level);
	...
	// 计算特征点的方向
	for (int level = 0; level < levels_num_; ++level)
		computeOrientation(vec_image_pyramid_[level], all_keypoints[level],
				umax_);
}
...
void ORBextractor::computeKeyPointsOld(
	...
	float image_ratio = (float) vec_image_pyramid_[0].cols
			/ vec_image_pyramid_[0].rows;

	for (int level = 0; level < levels_num_; ++level) {
		// 每层待检测的特征数
		const int desired_features = feature_num_per_level_[level];
		// 根据每层待检测特征点的个数确定划分格子的行列数
		const int level_cols = sqrt(
				(float) desired_features / (5 * image_ratio));
		const int level_rows = image_ratio * level_cols;

		const int min_border_x = EDGE_THRESHOLD;
		const int min_border_y = min_border_x;
		const int max_border_x = vec_image_pyramid_[level].cols
				- EDGE_THRESHOLD;
		const int max_border_y = vec_image_pyramid_[level].rows
				- EDGE_THRESHOLD;
		...
		// 得到每个格子需检测的特征数
		const int features_per_cell = ceil(
				(float) desired_features / cells_num);
		// 对应的每个格子的特征
		std::vector<std::vector<std::vector<cv::KeyPoint> > > cell_keypoints(
				level_rows,
				std::vector<std::vector<cv::KeyPoint> >(level_cols));
		// 用于存储每个格子保留的特征数
		std::vector<std::vector<int> > to_retain_num(level_rows,
				std::vector<int>(level_cols, 0));
		// 用于存储实际每个格子检测到的特征数
		std::vector<std::vector<int> > total_num(level_rows,
				std::vector<int>(level_cols, 0));
		// 用于标识特征数是否小于需求数
		std::vector<std::vector<bool> > is_no_more(level_rows,
				std::vector<bool>(level_cols, false));
		...
		for (int i = 0; i < level_rows; i++) {
			...
			for (int j = 0; j < level_cols; j++) {
				...
				// 对每个格子进行特征检测
				cv::Mat cell_image = vec_image_pyramid_[level].rowRange(ini_y,
						ini_y + max_y).colRange(ini_x, ini_x + max_x);

				cell_keypoints[i][j].reserve(features_per_cell * 5);

				cv::FAST(cell_image, cell_keypoints[i][j],
						default_fast_threshold_, true);
				// 如果特征个数较少，则降低阈值再一次检测
		...
		while (to_distribute_num > 0 && no_more_num < cells_num) {
			// 确定新的格子中的特征数
			int new_features_cell = features_per_cell
					+ ceil(
							(float) to_distribute_num
									/ (cells_num - no_more_num));
			to_distribute_num = 0;
				...
				for (int j = 0; j < level_cols; j++) {
					if (!is_no_more[i][j]) {
						if (total_num[i][j] > new_features_cell) {
							...
							to_distribute_num += new_features_cell
									- total_num[i][j];
							is_no_more[i][j] = true;
							no_more_num++;
						}
...
void ORBextractor::operator()(cv::InputArray image_array,
		cv::InputArray mask_array, std::vector<cv::KeyPoint>& output_keypoints,
	...
	for (int level = 0; level < levels_num_; ++level) {
		...
		GaussianBlur(working_mat, working_mat, cv::Size(7, 7), 2, 2,
				cv::BORDER_REFLECT_101);

		// 计算描述子
		cv::Mat desc = descriptors.rowRange(offset,
		...
		if (level != 0) {
			...
			for (std::vector<cv::KeyPoint>::iterator keypoint =
					keypoints.begin(), keypoint_end = keypoints.end();
					keypoint != keypoint_end; ++keypoint)
				keypoint->pt *= scale;
		}
		// 将完成好的特征点添加到输出vector中
		output_keypoints.insert(output_keypoints.end(), keypoints.begin(),
				keypoints.end());
	}
}
...
void ORBextractor::computePyramid(cv::Mat image) {
	for (int level = 0; level < levels_num_; ++level) {
		...
		if (level != 0) {
			resize(vec_image_pyramid_[level - 1], vec_image_pyramid_[level], sz,
					0, 0, cv::INTER_LINEAR);
		} else {
